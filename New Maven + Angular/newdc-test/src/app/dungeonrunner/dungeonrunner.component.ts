...
@Component({
  selector: 'app-dungeonrunner',
  templateUrl: './dungeonrunner.component.html',
  styleUrls: ['./dungeonrunner.component.css']
})
export class DungeonrunnerComponent implements OnInit {
  ...
  startingPosition: number[] = [6, 8]; //Testing, changed later

  currentPosition: number[] = this.startingPosition;
  dungeonDimensions: number[] = [25, 25]; //Testing, changed later
  scale: number = 75;
  ...
  openHelpMenu() {
    let dialogRef = this.helpMenuDialog.open(HelpmenuComponent);
  }
