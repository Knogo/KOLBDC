...
//    }
    ...
    //For new users
    @PostMapping(value = {"/newusers/", "/"}, consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Users> newUsers(@RequestBody
            @Valid
            final Users u, HttpSession session) {
        if (uDB.existsById(u.getID()) || uDB.existsByName(u.getName())) {
            ...
            logger.info("User:" + temp.getName() + " Pass: " + temp.getPassword());
